    ...
    <>
      ...
      <aside
        className={`
          ...
          bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border-r border-slate-100 dark:border-slate-800 shadow-sidebar
          flex flex-col
      ...
      >
        ...
        <div className="flex items-center gap-3 px-6 py-6 border-b border-slate-100 dark:border-slate-800">
          ...
          <div>
            <h1 className="text-lg font-bold text-slate-800 dark:text-white tracking-tight">MediCare</h1>
            <p className="text-xs text-slate-400 dark:text-slate-500 font-medium">Patient Management</p>
          </div>
          {/* Mobile close button */}
          <button
            onClick={onClose}
            className="ml-auto p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors lg:hidden cursor-pointer"
          ...
          <div className="space-y-1">
            {navItems.map((item) => (
              ...
              >
                {({ isActive }) => (
                  <>
                    <span className={`flex-shrink-0 transition-colors ${isActive ? 'text-primary-600 dark:text-primary-500' : 'text-slate-400 dark:text-slate-500 group-hover:text-slate-500 dark:group-hover:text-slate-400'}`}>
                      {item.icon}
                    </span>
