...
export default function PatientListing({ toast }) {
  ...
  const filteredPatients = useMemo(() => {
    return patients.filter((patient) => {
      ...
      // Visit Date Filter
      // Assuming visit date filter targets the patient's last visit (most recent vital)
                  ...
                  return (
                    <tr key={patient.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors last:border-b-0 group">
                      ...
                      <td className="px-6 py-4 text-right">
                        <Button variant="ghost" size="sm" onClick={() => navigate('/vitals', { state: { patientId: patient.id } })}>
                          Add Vitals
                        </Button>
                      </td>
                    </tr>
